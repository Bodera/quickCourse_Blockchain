...
namespace BlockchainServiceAlice.Models
{
    public class CryptoCurrency
    {
        ...
        public CryptoCurrency(){
            minerPrivateKey = "L3x9KHTRQWX9mpySSKKGWDdGALdSXJ579a9RYLxwN3ooSYRY9rwE"; //_minersWallet.PrivateKey;
            NodeId = "13zzd5rxeA19L3ph8Qm4RAW83i5KbZaBNu"; //_minersWallet.PublicKey;

            //string sign = RSA.RSA.Sing(minerPrivateKey, "h");
            //bool bol = RSA.RSA.Verify(NodeId, "h", sign);

            //initial transaction
            var transaction = new Transaction { Sender_Address = "0", Recipient_Address = NodeId, 
        ...
        private int CreateProofOfWork(string previousHash)
        {
            ...
            var transaction = new Transaction { Sender_Address = "0", Recipient_Address = NodeId,
                                                ...
                                                Signature = "" };
            _currentTransactions.Add(transaction);
            //CreateTransaction(sender_Address: "0",recipient: NodeId,amount: reward,signature: "");//reward
            blockCount++;
        ...
        private List<Transaction> TransactionByAddress(string ownerAddress)
        {
            ...
            foreach (var block in _chain.OrderByDescending(x => x.Index))
            {
                var ownerTransactions = block.Transactions.
                    Where(x => x.Sender_Address == ownerAddress || x.Recipient_Address == ownerAddress);
        ...
        {
            var transactions = TransactionByAddress(transaction.Sender_Address);
            decimal balance = 0;
            foreach (var item in transactions)
            {
                if (item.Recipient_Address == transaction.Sender_Address)
                    balance += item.Amount;
                else
                    balance -= item.Amount;
            }
        ...
        private void AddTransaction(Transaction transaction)
        {
            ...
            if (transaction.Sender_Address != NodeId)
        ...
        internal object CreateTransaction(Transaction transaction)
        {
            ...
            var verified = Verify_transaction_signature(transaction, transaction.Signature, transaction.Sender_Address);
            
            if (verified == false || transaction.Sender_Address == transaction.Recipient_Address)
            {
